...
def launch_ui_server(ui_directory: str, port: int):
    """
...
def launch_prompt_server(port: int = 8001):
    """
...
class PromptManager:
    ...
    def run(self):
        ...
        static_file_dir = pkg_resources.resource_filename(
            "constellaxion", "ui/prompts"
        )
        browser_port = 9000
        ...
        static_process = multiprocessing.Process(
            target=launch_ui_server,
            args=(static_file_dir, browser_port)
        )
        static_process.start()

        # Start API server in a separate process
        api_process = multiprocessing.Process(
            target=launch_prompt_server,
            args=(api_port,)
        )
        api_process.start()
