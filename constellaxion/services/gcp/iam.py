...
def assign_project_roles(project_id, service_account_email, roles):
    """
    Assign project-level roles to a service account, skipping roles that are
    already assigned.
    ...
    for role in roles:
        ...
        if binding:
            ...
            if f"serviceAccount:{service_account_email}" in members:
                print(
                    f"""Role {role} already assigned to {service_account_email}.
                    Skipping..."""
                )
                continue
            else:
...
def assign_impersonation_role(project_id, service_account_email, user_email):
    """
    Assign the Service Account User role to a user to allow them to act as the
    service account.
    ...
    if binding:
        ...
        if f"user:{user_email}" in members:
            print(
                f"""User {user_email} already has 'roles/iam.serviceAccountUser' on
                {service_account_email}. Skipping..."""
            )
            return
        else:
    ...
    print(
        f"""User {user_email} granted 'roles/iam.serviceAccountUser' on
